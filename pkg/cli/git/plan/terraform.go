...
var mainTFTemplate = `# Generated by Radius CLI - Do not edit manually
...
func (g *TerraformGenerator) generateContextFile(data *TerraformTemplateData) error {
	...
	sb.WriteString(fmt.Sprintf("Kubernetes Context: %s\n", data.KubernetesContext))
	sb.WriteString(fmt.Sprintf("Recipe Source: %s\n", data.RecipeSource))
	sb.WriteString("\nProperties:\n")

	if data.ResourceProperties != nil {
		propsJSON, _ := json.MarshalIndent(data.ResourceProperties, "  ", "  ")
		sb.WriteString("  ")
		sb.WriteString(string(propsJSON))
		sb.WriteString("\n")
	}

	if len(data.ResourceConnections) > 0 {
		sb.WriteString("\nConnections:\n")
		for name, conn := range data.ResourceConnections {
...
func (g *TerraformGenerator) InitAndPlan(ctx context.Context) (*PlanResult, error) {
	...
	// Parse plan for changes summary
	result := &PlanResult{
		HasChanges:  hasChanges,
		PlanOutput:  planOutput,
		PlanFile:    planTextPath,
		OutputDir:   g.OutputDir,
	}

	// Try to get structured plan for change counts
