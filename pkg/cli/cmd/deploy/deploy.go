...
func (r *Runner) generatePlan(ctx context.Context, template map[string]any) error {
	...
	for _, tmplRes := range sorted {
		...
		// Clean properties to remove ARM template expressions
		cleanedProps := cleanProperties(tmplRes.Properties, appName, envName)
		...
		step := DeploymentStep{
			...
			ExpectedChanges: StepChanges{
				Add:     1, // Default: expect 1 resource to be added
				Change:  0,
				Destroy: 0,
			},
		...
		// Create step directory and generate files (no /artifacts subdirectory per spec)
		stepDir := filepath.Join(planDir, dirName)
		if err := os.MkdirAll(stepDir, 0755); err != nil {
			return fmt.Errorf("failed to create step directory: %w", err)
		}

		r.Output.LogInfo("  Step %d: %s (%s) [%s]", stepNum, tmplRes.Name, tmplRes.Type, recipeKind)

		if recipeKind == "terraform" && recipeLocation != "" {
			if err := generateDeploymentArtifacts(stepDir, tmplRes, appName, envName, recipeLocation); err != nil {
...
// cleanProperties removes ARM template expressions from resource properties and
// replaces them with resolved values where possible.
func cleanProperties(props map[string]any, appName, envName string) map[string]any {
	if props == nil {
		return nil
	}
	...
	for k, v := range props {
		cleaned[k] = cleanValue(v, appName, envName)
	}
	return cleaned
}

// cleanValue recursively cleans a property value, resolving ARM template expressions.
func cleanValue(v any, appName, envName string) any {
	switch val := v.(type) {
	case string:
		// Check if it's an ARM template expression like [reference('name').id]
		if strings.HasPrefix(val, "[") && strings.HasSuffix(val, "]") {
			// Try to resolve common expressions
			if strings.Contains(val, "parameters('environment')") {
				return envName
			}
			if strings.Contains(val, "parameters('application')") {
				return appName
			}
			if strings.Contains(val, "reference(") && strings.Contains(val, ".id]") {
				// Extract the resource name from reference('name').id
				// Return a placeholder resource ID
				return "(resolved at deploy time)"
			}
			// Unknown expression, return placeholder
			return "(resolved at deploy time)"
		}
		return val
	case map[string]any:
		return cleanProperties(val, appName, envName)
	case []any:
		result := make([]any, len(val))
		for i, item := range val {
			result[i] = cleanValue(item, appName, envName)
		}
		return result
	default:
		return v
...
`, res.Name, res.Type, timestamp)
	...
	// Clean properties to remove ARM template expressions
	cleanedProps := cleanProperties(res.Properties, appName, envName)
	...
	// terraform-context.txt — human-readable deployment context (per spec appendix C.2)
...
`, timestamp, res.Name, res.Type, recipeLocation, appName, envName)
	...
	// tfplan.txt — placeholder for terraform plan output
	tfplanTxt := fmt.Sprintf(`# Terraform Plan Output
# Generated by Radius control plane
# Generated: %s
# Resource: %s (%s)
...
`, timestamp, res.Name, res.Type, safeName, safeName, res.Name)

	if err := os.WriteFile(filepath.Join(dir, "tfplan.txt"), []byte(tfplanTxt), 0644); err != nil {
		return err
	}
